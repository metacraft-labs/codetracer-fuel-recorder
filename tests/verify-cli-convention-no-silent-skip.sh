#!/usr/bin/env bash
# Verify that the codetracer-fuel-recorder CLI complies with
...
#     `CODETRACER_FUEL_RECORDER_DISABLED` are referenced in source so
...
assert_present "codetracer-fuel-recorder" "--version output" "${VERSION_OUT}"

# ---------------------------------------------------------------------------
# Source-level reference for the env-var fallback
# ---------------------------------------------------------------------------

# The recorder must reference CODETRACER_FUEL_RECORDER_OUT_DIR in
# source (otherwise the env-var fallback either doesn't exist or has
# been silently removed).  We grep recursively under src/.
if ! grep -rqF "CODETRACER_FUEL_RECORDER_OUT_DIR" "${REPO_ROOT}/src"; then
  echo "FAIL: CODETRACER_FUEL_RECORDER_OUT_DIR must be referenced in src/" >&2
  exit 1
fi
echo "ok: CODETRACER_FUEL_RECORDER_OUT_DIR referenced in src/"

if ! grep -rqF "CODETRACER_FUEL_RECORDER_DISABLED" "${REPO_ROOT}/src"; then
  echo "FAIL: CODETRACER_FUEL_RECORDER_DISABLED must be referenced in src/" >&2
  exit 1
fi
echo "ok: CODETRACER_FUEL_RECORDER_DISABLED referenced in src/"
